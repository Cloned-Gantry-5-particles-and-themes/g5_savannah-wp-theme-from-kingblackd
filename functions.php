...
if (!$gantry->isCompatible($requiredGantryVersion)) {
    ...
    $error = sprintf(__('Please upgrade Gantry 5 Framework to v%s (or later) before using %s theme!', 'g5_savannah'), strtoupper($requiredGantryVersion), $current_theme->get('Name'));
...
foreach ($helpers as $file) {
    if (!$filepath = locate_template($file)) {
        trigger_error(sprintf(__('Error locating %s for inclusion', 'g5_savannah'), $file), E_USER_ERROR);
    }
